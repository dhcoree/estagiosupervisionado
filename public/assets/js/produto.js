  ...
  try {
    ...
    for (const product of json.data) {
      rows += `
        ...
        product.description
      }', ${product.price}, ${product.quantidade}, '${product.fornecedor}', '${
        product.grupo
      }', '${
        product.marca
...
function update(_id, description, price, quantidade, fornecedor, grupo, marca) {
  ...
  for (const select of selects) {
    if (select.name == 'fornecedor') {
      const row = Array.prototype.map
        .call(select.options, m => m)
        .find(m => {
          return m.text === fornecedor
        })
      select.value = row.value
    } else if (select.name == 'grupo') {
      const row = Array.prototype.map
        .call(select.options, m => m)
        .find(m => {
          return m.text === grupo
