...
// Getting all fornecedors and update table
async function getAll() {
  try {
    ...
    // Create fornecedor payload for request
    const fornecedor = {};
...
function update(_id, nome, idade) {
  ...
  // Saving fornecedor to temporary variable
  toUpdate = {
    _id,
    nome,
    idade,
  ...
  try {
    ...
    if (confirm("Confirm Delete?")) {
      ...
      const response = await fetch(
          `${location.origin}/api/fornecedor/${id}`,
          options
        ),
...
async function submitForm(event) {
  ...
  try {
    ...
    // Create fornecedor object payload
    const fornecedor = {};

    // Setting fornecedor payload properties
    for (const input of inputs) {
      fornecedor[input.name] = input.value;
    }
...
function makeReport(table_id = "fornecedor_table", separator = ";") {
