...
// Getting all grupos and update table
async function getAll() {
  try {
    ...
    // Create grupo payload for request
    const grupo = {}
    ...
    for (const grupo of json.data) {
      rows += `
        <tr>
          <td class="${styles}">${grupo.nome}</td>
          <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium acoes">
            <button onclick="update('${grupo._id}', '${grupo.nome}')" class="text-indigo-600 hover:text-indigo-900 ">Editar</button>
            <button onclick="remove('${grupo._id}')" class="text-indigo-600 hover:text-indigo-900">Deletar</button>
          </td>
        </tr>
      `
    }
...
function update(_id, nome) {
  ...
  // Saving grupo to temporary variable
  toUpdate = {
    _id,
    nome
  }
...
async function submitForm(event) {
  ...
  try {
    ...
    // Create grupo object payload
    const grupo = {}

    // Setting grupo payload properties
    for (const input of inputs) {
...
function makeReport(table_id = 'grupo_table', separator = ';') {
