...
async function getAll() {
  try {
    ...
    // Mount URL Request
    let urlRequest = `${location.origin}/api/vendas`
...
async function remove(id) {
  try {
    ...
    if (confirm('Confirm Delete?')) {
      ...
      const response = await fetch(`${location.origin}/api/product/${id}`, options),
...
async function submitForm(event) {
  ...
  try {
    ...
    // Create URL Request
    let urlRequest = `${location.origin}/api/product`

    // Verify if it is update
    if (toUpdate)
      urlRequest += `/${toUpdate._id}`
