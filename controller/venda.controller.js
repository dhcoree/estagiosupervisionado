...
route.get('/', async (request, response) => {
  try {
    ...
    let vendas = await venda.read(payload)

    // trocando os ids dos marcas pelos nomes
    let produtos = await produtoModel.read({})
    let marcas = await marcaModel.read({})

    vendas = vendas.map(v => {
      let produto = produtos.find(f => f.description === v.produto)
      let marca = marcas.find(f => f._id === produto.marca)

      if (!marca) marca = ''
      else marca = `${marca.nome}`
